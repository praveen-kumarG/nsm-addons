...
class account_invoice_verifier(osv.osv_memory):
    """
    ...
    def invoice_verifier(self, cr, uid, ids, context=None):
        ...
        for record in data_inv:
            if record['state'] != ('auth'):
                raise osv.except_osv(_('Warning!'), _("Selected invoice(s) cannot be verified as they are not in 'Authorized' state."))
            wf_service.trg_validate(uid, 'account.invoice', record['id'], 'invoice_verify', cr)
